			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Number") === oObject.Number;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Number;
										} else {
											sCurrentId = aItemsNotInTheList[0].Number;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("SO_header1Set", {
											Number : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("Number"),
							title: oBindingContext.getProperty("Number")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all SO_header1Set to be able to check for the total amount
